...
if ($xml !== false && strcasecmp($xml->call, $_SERVER['PHP_AUTH_USER']) == 0) {
	...
	try {
		...
		$sql_scores = 'INSERT INTO scores (callsign, lst_upd, score, mult, qso, arrl_section, cq_zone, club, mode, bands, ops, xmtrs, power) VALUES (:callsign, :lstUpd, :score, :mult, :qso, :arrlSection, :cqZone, :club, :mode, :bands, :ops, :xmtrs, :power)';
		...
		$sql_scores = 'INSERT INTO scores_current (callsign, lst_upd, score, mult, qso, arrl_section, cq_zone, club, mode, bands, ops, xmtrs, power) VALUES (:callsign, :lstUpd, :score, :mult, :qso, :arrlSection, :cqZone, :club, :mode, :bands, :ops, :xmtrs, :power) ON DUPLICATE KEY UPDATE lst_upd=VALUES(lst_upd), score=VALUES(score), mult=VALUES(mult), qso=VALUES(qso), arrl_section=VALUES(arrl_section), cq_zone=VALUES(cq_zone), club=VALUES(club), mode=VALUES(mode), bands=VALUES(bands), ops=VALUES(ops), xmtrs=VALUES(xmtrs), power=VALUES(power)';
		$stmt = $db->prepare($sql_scores);
		$stmt->bindParam('callsign', $callsign);
		$stmt->bindParam('lstUpd', $xml->timestamp);
		$stmt->bindParam('score', $score);
